...
Example implementation:
    %pull(startyear=2016, endyear=2016, startmonth=10,endmonth=10,startdate=21,enddate=31);
    ...
    * Note: this code only works for post-10/21/2016 TAQ data. Please see "Retail_trades_pre201610.sas"
    for code to calculate variables for trades in pre-10/21/2016 TAQ data.
**********************************************************************************;

/* Temp Library Name to save the final SAS dataset */
libname project '[YOUR WRDS TEMP FOLDER PATH HERE]'; 
options errors=50;
option msglevel=i mprint source mlogic; 
...
DATA project.&TradesAll;
 LENGTH SYM_ROOT $ 6 DATE 4 sumsize 8 sumtrades 8 sumtrades 8 buy_volume 8 buy_trades 8 sell_volume 8 sell_trades 8 small_volume 8 small_trades 8 med_volume 8 med_trades 8
...
%do y=&startyear %to &endyear; /*cycling through years*/
	%do m=&startmonth %to &endmonth; /*cycling through months*/
		...
		%do d=&startdate %to &enddate; /*cycling through days*/
            ...
            %put 'The date should be' &y&prefixm&m&prefixd&d;
...
/*This is where you actually specify the years, months, days and then run the macro. 
I typically run one or just a few months at a time. The dates have to be consecutive and the startmonth
...
%pull(startyear=2017, endyear=2017, startmonth=7,endmonth=7,startdate=1,enddate=31);*/
